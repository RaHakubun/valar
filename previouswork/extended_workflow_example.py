...
if __name__ == "__main__":
    # 指定 YAML 文件路径
    yaml_file_path = "extended_nodes_example.yaml"
    ...
    try:
        ...
        # 示例3: 使用ControlNet的工作流
        controlnet_workflow = [
            'CheckpointLoaderSimple',
            'ControlNetLoader',    # 新增的ControlNet节点
            'LoadImage',           # 假设我们有这个节点定义
            'CannyEdgePreprocessor', # 新增的预处理器节点
            'CLIPTextEncode',
            'CLIPTextEncode',
            'ControlNetApply',     # 新增的ControlNet应用节点
            'KSampler',
            'VAEDecode',
            'SaveImage'
        ]
        ...
        upscale_workflow = [
            ...
            'KSampler',
            'VAEDecode',
            'UpscaleModelLoader',      # 新增的Upscaler节点
            'ImageUpscaleWithModel',   # 新增的Upscaler应用节点
            'SaveImage'
        ]
        ...
        print("示例1: 基础文生图工作流")
        print("="*50)
        result = generate_function_calls(basic_workflow, yaml_data).strip()
        print(result)
        print("\n")
        
        print("示例2: 使用LoRA的工作流")
        print("="*50)
        result = generate_function_calls(lora_workflow, yaml_data).strip()
        print(result)
        print("\n")
        
        print("示例3: 使用ControlNet的工作流")
        print("="*50)
        result = generate_function_calls(controlnet_workflow, yaml_data).strip()
        print(result)
        print("\n")
        
        print("示例4: 使用Upscaler的工作流")
        print("="*50)
        result = generate_function_calls(upscale_workflow, yaml_data).strip()
        print(result)
        print("\n"